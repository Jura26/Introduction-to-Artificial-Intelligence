    ...
    try:
        if language == "python":
            ...
            if not os.path.exists("pom.xml"):
                shutil.copy(
                    os.path.join(
                        cwd, "data", lab, "templates", f"{lab}java", "pom.xml"
                    ),
                    path_to_solution,
                )
...
def validate_solution_structure(solution_folder):
    ...
    # the programming language of the solution (lab[x][java/py])
...
def iterate_student_solutions(
    ...
    for idx, dir_name in enumerate(sorted(solutions)):
        ...
        files = [
            f for f in files if f != "autograder.log"
        ]  # if autograder.log already exists skip it
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            if not success:
                ...
                with open(
                    os.path.join(student_dir, "autograder.log"), "wt", encoding="utf-8"
                ) as student_log:
...
def run_evaluation(
    ...
    for subtask in test_suite:
        ...
        for test_instance in test_suite[subtask]:
            ...
            evaluation_fields = [
                field
                for field in test_instance["expected_output_fields"]
                if test_instance["expected_output_fields"][field]["match"] != "ignored"
            ]
            ...
            result_code, result, command = execute(
                path_to_solution, language, execution_args
            )
            ...
            if result_code == ERR_EXECUTE:
                ...
                field_matches = grade_solution(
                    student_output, test_instance["expected_output_fields"]
                )
